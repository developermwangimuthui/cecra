...
class ProductController extends App\Http\Controllers\Seller\SellerController {
    ...
    public function fetch() {
        ...
        $shopId = session()->get('businessUserId');

        $shopProducts = VendorProduct::where('vendor_id', $shopId)->get();

        $this->results = $shopProducts;
        ...
        if($this->results) {
            ...
            foreach($this->rows as $row) {
                ...
                $this->data[$i][] = '<td class="text-right">
                                        ...
                                        <a href="'.route('shop.product.delete',[$row['id']]).'" class="btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash"></i></a>
    ...
    public function setData($request) {
        ...
        $mimi = Helper::resize('no_image.png', 100, 100);
//        dd($mimi);
            ...
            if(count($this->data['colors'])) {
                foreach ($this->data['colors'] as $color) {
                    VendorProductColor::create([
                        'product_color'     =>  $color,
                        'vendor_product_id' =>  $vendorProduct->vendor_product_id
                    ]);
                }
            }
