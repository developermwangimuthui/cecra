...
class MenuController extends BaseController
{
    ...
    public function fetchDish () {
        ...
        $restaurantId = session()->get('businessUserId');

        $restaurantDishes = Dish::where('vendor_id', $restaurantId)->get();

        $this->results = $restaurantDishes;
        // dd($this->results);
        ...
        if($this->results) {
            ...
            foreach($this->rows as $row) {
                ...
                $this->data[$i][] = '<td class="">
                                            
                                            <a type="button" href="'.route('dish.edit',[$row['id']]). '" class="btn btn-info btn-sm" title="Edit"><i class="fas fa-edit"></i></a>
                                            <a type="button" href="' . route('dish.delete', [$row['id']]) . '" class="btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash"></i></a>
...
//        }
    ...
    public function store(Request $request) {
        try {
            ...
            // dd($dish);
            $imagesPut = $this->data['images'];

            // dd($this->data['images'][0]);


            session()->put('dish_info', [
                'name' => $dish->name,
                // 'image' => ($this->data['images'][0]) ? asset('uploads/' . $this->data['images'][0]) : asset('assets/images/users/user.png'),
    ...
    public function delete($id){
        ...
        session()->flash('message', trans('sentence.restaurant.menu.label.Product deleted successfully'));
